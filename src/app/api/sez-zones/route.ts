...
// Dev-only endpoint: persists edited SEZ zone perimeters back to coords JSON.
...
export async function POST(req: Request) {
  ...
  for (const [key, pts] of Object.entries(coords as Record<string, unknown>)) {
    if (
      ...
      return NextResponse.json({ error: `Bad points for "${key}"` }, { status: 400 });
    }
  }

  const file = path.join(process.cwd(), 'src', 'data', 'sezZones.coords.json');
  await writeFile(file, JSON.stringify(coords, null, 0), 'utf8');

  return NextResponse.json({ ok: true, zones: Object.keys(coords).length });
}
